...
if ($order->content_type == 'virtual' || ($order->content_type == 'virtual_weight') || ($_SESSION['cart']->count_contents_virtual() == 0)) { // GV Code added
  ...
  if (twe_not_null($order->billing['country']['iso_code_2'])) {
  ...
  if ( defined('MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING') && (MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING == 'true') ) {
    ...
    $free_shipping = false;
//    購物滿額免運費 bug, 先把運費扣除避免計算錯誤 20150209
...
$smarty->assign('FORM_ACTION',twe_draw_form('checkout_address', twe_href_link(FILENAME_CHECKOUT_SHIPPING, '', 'SSL'),'post','class="form-horizontal" role="form"') . twe_draw_hidden_field('action', 'process'));
...
if (twe_count_shipping_modules() > 0) {
	...
	if ($free_shipping == true) {
		...
		$module_smarty->assign('FREE_SHIPPING_DESCRIPTION', sprintf(FREE_SHIPPING_DESCRIPTION, $currencies->format(MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING_OVER)).twe_draw_hidden_field('shipping', 'free_free'));
		$module_smarty->assign('FREE_SHIPPING_ICON', $quotes[$i]['icon']);
	} else {
		$radio_buttons = 0;
		#loop through installed shipping methods...
		for ($i = 0, $n = sizeof($quotes); $i < $n; $i ++) {
			$quotes[$i]['module'] = $quotes[$i]['module'].$quotes[$i]['icon'];
			if (!isset ($quotes[$i]['error'])) {
				for ($j = 0, $n2 = sizeof($quotes[$i]['methods']); $j < $n2; $j ++) {
					...
					if (($n > 1) || ($n2 > 1)) {
						...
						$quotes[$i]['methods'][$j]['price'] = twe_format_price(twe_add_tax($quotes[$i]['methods'][$j]['cost'], $quotes[$i]['tax']),$price_special=1,$calculate_currencies=true);						
            $quotes[$i]['methods'][$j]['radio_field'] = twe_draw_radio_field('shipping', $quotes[$i]['id'].'_'.$quotes[$i]['methods'][$j]['id'], $checked, 'id="'.($i+1).'"');
					} else {
						if ($_SESSION['customers_status']['customers_status_show_price_tax']==0)  $quotes[$i]['tax']=0;
            $quotes[$i]['methods'][$j]['price'] = twe_format_price(twe_add_tax($quotes[$i]['methods'][$j]['cost'], $quotes[$i]['tax']),$price_special=1,$calculate_currencies=true).twe_draw_hidden_field('shipping', $quotes[$i]['id'].'_'.$quotes[$i]['methods'][$j]['id']);
					}
					$radio_buttons ++;
				}
			}
