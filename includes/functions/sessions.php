...
if (!is_object($db)) {
 ...
 function twe_session_start() {
  ...
  if (isset($_GET[twe_session_name()])) {
    if (preg_match('/^[a-zA-Z0-9]+$/', $_GET[twe_session_name()]) == false) {
      unset($_GET[twe_session_name()]);

      $sane_session_id = false;
    }
  } elseif (isset($_POST[twe_session_name()])) {
    if (preg_match('/^[a-zA-Z0-9]+$/', $_POST[twe_session_name()]) == false) {
      unset($_POST[twe_session_name()]);

      $sane_session_id = false;
    }
  } elseif (isset($_COOKIE[twe_session_name()])) {
    if (preg_match('/^[a-zA-Z0-9]+$/', $_COOKIE[twe_session_name()]) == false) {
      $session_data = session_get_cookie_params();

      setcookie(twe_session_name(), '', time()-42000, $session_data['path'], $session_data['domain']);
  ...
  if ($sane_session_id == false) {
    twe_redirect(twe_href_link(FILENAME_DEFAULT, '', 'NONSSL', false));
  }
