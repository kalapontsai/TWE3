...
if (!$module_smarty->is_cached(CURRENT_TEMPLATE.'/module/center_modules/products_best.html', $cache_id) || !$cache) {
	$rebuild = true;
...
$products_new_query_raw = "SELECT * FROM
			                                         ...
			                                         " . $group_check . "
			                                         " . $fsk_lock . "
													                     and p.products_quantity > 0 and p.products_ordered > 0
			                                         and p.products_status = '1' and pd.language_id = '" . (int) $_SESSION['languages_id'] . "'
			                                         order by p.products_ordered DESC";
  $products_new = $db->Execute($products_new_query_raw,MAX_DISPLAY_INDEX_BESTSELLERS,SQL_CACHE,CACHE_LIFETIME);
...
$module_content = array();
 ...
 if (!$cache || $rebuild) {
	if (twe_not_null($module_content)) {
			//if ($rebuild)  $module_smarty->clear_cache(CURRENT_TEMPLATE.'/module/center_modules/products_best.html', $cache_id);
  $module= $module_smarty->fetch(CURRENT_TEMPLATE.'/module/center_modules/products_best.html',$cache_id);
  $default_smarty->assign('products_best',$module);
	}
} else {
  $module= $module_smarty->fetch(CURRENT_TEMPLATE.'/module/center_modules/products_best.html',$cache_id);
  $default_smarty->assign('products_best',$module);
