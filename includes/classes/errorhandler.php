<?php
// $Id: errorhandler.php,v 1.1 2004/07/28 23:10:49 oldpa Exp $
//  ------------------------------------------------------------------------ //
...
class tweErrorHandler
{
	 ...
	 */
	function tweErrorHandler()
	{
		set_error_handler('tweErrorHandler_HandleError');
		register_shutdown_function('tweErrorHandler_Shutdown'); 
	}
	...
	function renderErrors()
	{
		...
		if ($this->_isFatal) {
			...
			$output .= 'If you are the administrator of this site, please visit the <a href="http://www.oldpa.com.tw">TWE-Commerce</a> for assistance.<br/><br/>';
		}
 ...
 * @return void 
 */
function tweErrorHandler_HandleError($errNo, $errStr, $errFile, $errLine)
{ 
	// NOTE: we only store relative pathnames
	$new_error = array( 
		'errno' => $errNo,
		'errstr' => $errStr,
		'errfile' => preg_replace("|^" . DIR_WS_CATALOG . "/|", '', $errFile),
		'errline' => $errLine 
		);
	$error_handler =& tweErrorHandler::getInstance();
 ...
 * @return void 
 */
function tweErrorHandler_Shutdown()
{
	$error_handler =& tweErrorHandler::getInstance();
