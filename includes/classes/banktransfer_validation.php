<?php
/* -----------------------------------------------------------------------------------------
   $Id: banktransfer_validation.php,v 1.1 2003/09/06 22:13:53 oldpa   Exp $   
...
// Diese function gibt die Bankinformationen aus der Datenbank zurück*/
  function db_query($blz) {
    $blz_query = $db->Execute(
    "SELECT * from banktransfer_blz WHERE blz = '" . $blz . "'");
    if (twe_db_num_rows($blz_query))
      $data = twe_db_fetch_array ($blz_query);
    else
      $data = -1;
    return $data;
  }
