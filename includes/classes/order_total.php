<?php
/* -----------------------------------------------------------------------------------------
   $Id: order_total.php,v 1.2 2004/02/17 21:13:26 oldpa   Exp $   
  ...
  class order_total {
            ...
            //
                function credit_selection() {
                  ...
                  if (MODULE_ORDER_TOTAL_INSTALLED) {
                    ...
                    $header_string .= '  </tr>' . "\n";
                    $header_string .= '<tr>' . "\n";
                    $header_string .= '   <td><table border="0" width="100%" cellspacing="1" cellpadding="2">' . "\n";
                    $header_string .= '     <tr class="infoBoxContents"><td><table border="0" width="100%" cellspacing="0" cellpadding="2">' ."\n";
                    $header_string .= '       <tr><td width="10">' .  twe_draw_separator('pixel_trans.gif', '10', '1') .'</td>' . "\n";
                    $header_string .= '           <td colspan="2"><table border="0" width="100%" cellspacing="0" cellpadding="2">' . "\n";
                    $close_string   = '                           </table></td>';
                    $close_string  .= '<td width="10">' .  twe_draw_separator('pixel_trans.gif', '10', '1') . '</td>';
                    $close_string  .= '</tr></table></td></tr></table></td>';
                    $close_string  .= '<tr><td width="100%">' .  twe_draw_separator('pixel_trans.gif', '100%', '10') . '</td></tr>';
                    reset($this->modules);
                    $output_string = '';
                    while (list(, $value) = each($this->modules)) {
                      ...
                      if ($GLOBALS[$class]->enabled && $GLOBALS[$class]->credit_class) {
                        ...
                        if ( ($use_credit_string !='' ) || ($selection_string != '') ) {
                         $output_string .=  '<tr colspan="4"><td colspan="4" width="100%">' .  twe_draw_separator('pixel_trans.gif', '100%', '10') . '</td></tr>';
                          $output_string = ' <tr>' . "\n" .
                                           '   <td s width="10">' .  twe_draw_separator('pixel_trans.gif', '10', '1') .'</td>' .
                                          '     <td   class="main"><b>' . $GLOBALS[$class]->header .'</b></td>'.$use_credit_string;
                          $output_string .= '<td  width="10">' . twe_draw_separator('pixel_trans.gif', '10', '1') . '</td>';
                          $output_string .= '  </tr>' . "\n";
    ...
    function order_total() {
      ...
      if (defined('MODULE_ORDER_TOTAL_INSTALLED') && twe_not_null(MODULE_ORDER_TOTAL_INSTALLED)) {
        ...
        while (list(, $value) = each($this->modules)) {
          ...
          if ($GLOBALS[$class]->enabled) {
            ...
            for ($i=0, $n=sizeof($GLOBALS[$class]->output); $i<$n; $i++) {
              if (twe_not_null($GLOBALS[$class]->output[$i]['title']) && twe_not_null($GLOBALS[$class]->output[$i]['text'])) {
