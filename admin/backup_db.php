<?php
/* --------------------------------------------------------------
   $Id: backup_db.php,v 1.1 2010/12/19 13:19:08 oldpa Exp$
...
define ('RESTORE_TEST', false); //Standard: false - auf true ndern fr Simulation fr die Wiederherstellung, die SQL Befehle werden in eine Protokolldatei (log) im Backup-Verzeichnis geschrieben
...
if ($_GET['action'] == 'restorenow') {
	...
	//Protokollfatei lschen wenn sie schon existiert
...
if ($restore['file'] != '' && $_GET['action'] != 'restorenow'){
	...
	$table_ok= TEXT_TABLELINE . $tabellen_fertig  . '<br><br>'.TEXT_TABLELINE_NAME. $restore['actual_table'] . '<br><br>' . TEXT_RELOAD. $restore['aufruf'] ;
	...
	if ($restore['fileEOF'])  {
		...
		$table_ok= TEXT_TABLELINE . $tabellen_fertig .  '<br><br>' . TEXT_RELOAD. $restore['aufruf'] ;
		$button_back = '<a href="backup.php" class="button">'.twe_image_button('button_back.gif', IMAGE_BACK).'</a>';
		$selbstaufruf = '';
		//echo $restore['test'];
		$restore= array();
		unset($_SESSION['restore']);
...
if ($dump['num_tables'] > 0 && $_GET['action'] != 'backupnow'){
	...
	if ($dump['nr'] < $dump['num_tables']) {
		...
		$table_ok = TEXT_TABLELINE . ($nr + 1) .  '<br><br>'.TEXT_TABLELINE_NAME . $dump['tables'][$nr] . '<br><br>'.TEXT_RELOAD. $dump['aufruf'] ;
	...
	//Fertig
		...
		$table_ok= TEXT_TABLELINE . $dump['nr'] .  '<br><br>'. TEXT_RELOAD . $dump['aufruf'] ;
		$button_back = '<a href="backup.php" class="button">'.twe_image_button('button_back.gif', IMAGE_BACK).'</a>';
		$selbstaufruf = '';	    		
	    unset ($_SESSION['dump']);
		$button_back = '<a href="backup.php" class="button">'.twe_image_button('button_back.gif', IMAGE_BACK).'</a>';
