<?php
/* --------------------------------------------------------------
   $Id: html_graphs.php,v 1.1 2003/09/06 22:05:29 oldpa   Exp $   
  ...
  function twe_banner_graph_infoBox($banner_id, $days) {
  global $db;
  ...
  ////
  // draws a double vertical bar graph for the banner views vs clicks statistics
  function twe_banner_graph_yearly($banner_id) {
    global $banner,$db;

    $banner_stats = $db->Execute("select year(banners_history_date) as year, sum(banners_shown) as value, sum(banners_clicked) as dvalue from " . TABLE_BANNERS_HISTORY . " where banners_id = '" . $banner_id . "' group by year(banners_history_date)");
    while (!$banner_stats->EOF) {
  ...
  ////
  // draws a double vertical bar graph for the banner views vs clicks statistics
  function twe_banner_graph_monthly($banner_id) {
    global $banner,$db;
    ...
    $banner_stats = $db->Execute("select month(banners_history_date) as banner_month, sum(banners_shown) as value, sum(banners_clicked) as dvalue from " . TABLE_BANNERS_HISTORY . " where banners_id = '" . $banner_id . "' and year(banners_history_date) = '" . $year . "' group by month(banners_history_date)");
    while (!$banner_stats->EOF) {
  ...
  ////
  // draws a double vertical bar graph for the banner views vs clicks statistics
  function twe_banner_graph_daily($banner_id) {
    global $banner,$db;
