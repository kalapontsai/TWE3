<?php
/* --------------------------------------------------------------
   $Id: application_top.php,v 1.26 2004/04/25 16:30:44 oldpa Exp $
...
--------------------------------------------------------------*/
 //include('includes/debugging.php');
  ...
  // Disable use_trans_sid as twe_href_link() does this manually
...
if (version_compare(PHP_VERSION, '5.1.0', '>=')) {
  ...
  define('FILENAME_ORDER_EDIT', 'orders_edit.php');
  ...
  define('FILENAME_EZSHIP', 'ezship.php');
...
//START MASTER PRODUCTS
 ...
 if (STORE_SESSIONS != 'mysql')  session_save_path(SESSION_WRITE_DIRECTORY);
  ...
  if (SESSION_FORCE_COOKIE_USE == 'True') {
    twe_setcookie('cookie_test', 'please_accept_for_session', time()+60*60*24*30, '/', $cookie_domain);
...
//    $user_info['user_ip_date'] =  value will be in fact added when login ;
  ...
  twe_get_customers_statuses();


if (!strstr($PHP_SELF, 'Adminlogin.php')) {
  $pagename = strtok($current_page, '.');
  if (!isset($_SESSION['customer_id'])) {
    twe_redirect(twe_href_link(FILENAME_LOGIN));
  }

  if (twe_check_permission($pagename) == '0') {
    twe_redirect(twe_href_link(FILENAME_LOGIN));
  }
}
//if (version_compare(PHP_VERSION, '5.0.0', '>=')) {
  // require(DIR_FS_CATALOG.DIR_WS_CLASSES . 'Smarty_3.1.19/Smarty.class.php');
//}else{
