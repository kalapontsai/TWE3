<?php
/* --------------------------------------------------------------
   $Id: shopping_cart.php,v 1.1 2003/09/06 22:05:29 oldpa   Exp $   
    ...
    function restore_contents() {
      ...
      if ($this->contents) {
        ...
        while (list($products_id, ) = each($this->contents)) {
          ...
          $product_query = $db->Execute("select products_id from " . TABLE_CUSTOMERS_BASKET . " where customers_id = '" . $_SESSION['customer_id'] . "' and products_id = '" . $products_id . "'");
          if (!$product_query->RecordCount()) {
      ...
      $products = $db->Execute("select products_id, customers_basket_quantity from " . TABLE_CUSTOMERS_BASKET . " where customers_id = '" . $_SESSION['customer_id'] . "'");
      while (!$products->EOF) {
...
global $db;
      ...
      if ($this->in_cart($products_id)) {
        ...
        if ($_SESSION['customer_id']) $db->Execute("insert into " . TABLE_CUSTOMERS_BASKET . " (customers_id, products_id, customers_basket_quantity, customers_basket_date_added) values ('" . $_SESSION['customer_id'] . "', '" . $products_id . "', '" . $qty . "', '" . date('Ymd') . "')");
        ...
        if (is_array($attributes)) {
          ...
          while (list($option, $value) = each($attributes)) {
            ...
            // insert into database
            if ($_SESSION['customer_id']) $db->Execute("insert into " . TABLE_CUSTOMERS_BASKET_ATTRIBUTES . " (customers_id, products_id, products_options_id, products_options_value_id) values ('" . $_SESSION['customer_id'] . "', '" . $products_id . "', '" . $option . "', '" . $value . "')");
          }
        }
    ...
    function update_quantity($products_id, $quantity = '', $attributes = '') {
     global $db;
      ...
      // update database
      if ($_SESSION['customer_id']) $db->Execute("update " . TABLE_CUSTOMERS_BASKET . " set customers_basket_quantity = '" . $quantity . "' where customers_id = '" . $_SESSION['customer_id'] . "' and products_id = '" . $products_id . "'");
      ...
      if (is_array($attributes)) {
        ...
        while (list($option, $value) = each($attributes)) {
          $this->contents[$products_id]['attributes'][$option] = $value;
          // update database
          if ($_SESSION['customer_id']) $db->Execute("update " . TABLE_CUSTOMERS_BASKET_ATTRIBUTES . " set products_options_value_id = '" . $value . "' where customers_id = '" . $_SESSION['customer_id'] . "' and products_id = '" . $products_id . "' and products_options_id = '" . $option . "'");
        }
      }
    }
...
global $db;
	...
	global $db;
      ...
      while (list($products_id, ) = each($this->contents)) {
        ...
        // products price
        $product_query = "select products_id, products_price, products_tax_class_id, products_weight from " . TABLE_PRODUCTS . " where products_id='" . twe_get_prid($products_id) . "'";
        if ($product = $db->Execute($product_query)) {
          ...
          $specials_query = "select specials_new_products_price from " . TABLE_SPECIALS . " where products_id = '" . $prid . "' and status = '1'";
          ...
          $this->total += twe_add_tax($products_price, $products_tax) * $qty;
        ...
        if ($this->contents[$products_id]['attributes']) {
          ...
          while (list($option, $value) = each($this->contents[$products_id]['attributes'])) {
            ...
            if ($attribute_price->fields['price_prefix'] == '+') {
              $this->total += $qty * twe_add_tax($attribute_price->fields['options_values_price'], $products_tax);
            } else {
              $this->total -= $qty * twe_add_tax($attribute_price->fields['options_values_price'], $products_tax);
            }
          }
	...
	global $db;
      if ($this->contents[$products_id]['attributes']) {
        reset($this->contents[$products_id]['attributes']);
        while (list($option, $value) = each($this->contents[$products_id]['attributes'])) {
          $attribute_price_query = "select options_values_price, price_prefix from " . TABLE_PRODUCTS_ATTRIBUTES . " where products_id = '" . $products_id . "' and options_id = '" . $option . "' and options_values_id = '" . $value . "'";
          $attribute_price = $db->Execute($attribute_price_query);
          if ($attribute_price->fields['price_prefix'] == '+') {
    ...
    global $db;
      ...
      reset($this->contents);
      while (list($products_id, ) = each($this->contents)) {
        $products_query = "select p.products_id, pd.products_name, p.products_model, p.products_price, p.products_weight, p.products_tax_class_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id='" . twe_get_prid($products_id) . "' and pd.products_id = p.products_id and pd.language_id = '" . $_SESSION['languages_id'] . "'";
