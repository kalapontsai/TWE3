<?php
/* ----------------------------------------------------------------------------------------
   $Id: image_manipulator.php,v 1.5 2004/03/16 19:24:49 oldpa Exp $   
...
class image_manipulation
	...
	function compile()
		...
		if($this->s !== Null)
			{
			// image_fixed_size contribution
			if (PRODUCT_IMAGE_FIXED_SIZE=='true') {
				$this->t = imagecreatetruecolor($this->m, $this->n); // created thumbnail reference
				$color = ImageColorAllocate($this->t,$this->hex2rgb(substr(PRODUCT_IMAGE_FIXED_SIZE_BACKGROUND, 0, 2)),$this->hex2rgb(substr(PRODUCT_IMAGE_FIXED_SIZE_BACKGROUND, 2, 2)),$this->hex2rgb(substr(PRODUCT_IMAGE_FIXED_SIZE_BACKGROUND, 4, 2)));
				@imagefill($this->t,0,0,$color);
				$this->u = imagecopyresampled($this->t, $this->s, round(($this->m-$this->q)/2), round(($this->n-$this->r)/2), 0, 0, $this->q, $this->r, $this->i, $this->j);
			}
			else {
			...
			// image_fixed_size contribution ends here
			}
		}
