...
/* -----------------------------------------------------------------------------------------
   $Id: ezship.php,v 0.2 2015/03/25 ELHOMEO.com   
   -----------------------------------------------------------------------------------------
...
*/
  ...
  // include needed functions
    ...
    if (isset($_POST['st_code']))   {
      $rtn_message = '體系代號:' . $_POST['st_cate'] . '<br>店號:' . $_POST['st_code'] . '<br>店名:' . $_POST['stName']. '<br>地址:' . $_POST['stAddr'];
      $smarty->assign('rtn_message',$rtn_message);
      $smarty->assign('st_cate_code',$_POST['st_cate'].$_POST['st_code']);
//    print_r($_REQUEST);
    }
    ...
    if (isset($_GET['oID']) || isset($_POST['webPara'])) {
      if (isset($_GET['oID'])) { $oID = $_GET['oID'];}
      if (isset($_POST['webPara'])) { $oID = $_POST['webPara'];}
      $order = new order($oID);
...
// 判斷是否使用便利商店  BEGIN
    ...
    if (isset($order->delivery['country']) && twe_not_null($order->delivery['country'])) {
      $country = twe_output_string_protected($order->delivery['country']);
          } else {
      $country = '';
      }
      $address = $country . '省' . $order->delivery['state'] . $order->delivery['city'] . $order->delivery['street_address'];
      $smarty->assign('address_rv_addr',$address);       
      $smarty->assign('rv_zip',$order->delivery['postcode']);
    } 
// 判斷是否使用便利商店  EOF
...
// 插入客戶意見 
    ...
    $total = round($total,0);
    if ( $total > 6000 ) $smarty->assign('ttl_message','超過金額!!');
